...
def test_submission_is_one_fixed_three_seed_l20_array() -> None:
    ...
    assert "submit_phase2_post_recovery_array_once.py" in submit
    ...
    assert "PRORM_PHASE2_ARRAY_CONCURRENCY" not in submit
    assert "array_selection" not in submit
    assert "configs/common_beta_post_recovery_calibration.yaml" in submit
    assert 'configs/common_beta_pilot.yaml"' not in submit
    assert "validate_phase2_recovery_authorization.py" in submit
    assert "authorization must be the locked recovery receipt" in submit
    assert "issue_schedule_frozen_full_common_beta_calibration_pilot" in submit
...
def test_container_mounts_only_job_writable_and_immutable_inputs_read_only() -> None:
    job = JOB.read_text(encoding="utf-8")
    bind = (
        '--bind "${job_dir}:${job_dir},${PRORM_HF_CACHE}:'
        "${PRORM_HF_CACHE}:ro,${PRORM_RECOVERY_AUTHORIZATION}:"
        '${PRORM_RECOVERY_AUTHORIZATION}:ro"'
    )
    assert bind in job
...
def test_job_revalidates_auth_and_detached_commit_before_and_after() -> None:
    ...
    assert job.count("validate_phase2_recovery_authorization.py") == 3
...
def test_aggregate_is_gated_by_terminal_evidence_and_has_no_project_rw_bind() -> None:
    ...
    assert "authorization must be the locked recovery campaign receipt" in submit
    ...
    assert "phase2-post-recovery-calibration-aggregate.json" in submit
