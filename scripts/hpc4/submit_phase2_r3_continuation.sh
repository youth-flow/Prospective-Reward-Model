#!/usr/bin/env bash
# Run Gate-R continuation validation on an HPC4 CPU compute node.

set -euo pipefail
umask 077

die() {
  printf 'R3 continuation submission launcher fatal: %s\n' "$*" >&2
  exit 1
}
...
if [[ -n "${PRORM_R3_PREVIOUS_CONTINUATION_PLAN:-}" || \
  ...
  optional_environment=(
    PRORM_R3_PREVIOUS_CONTINUATION_PLAN
...
driver="${repo_root}/scripts/hpc4/phase2_r3_continuation_submission.sbatch"
[[ -f "${driver}" && ! -L "${driver}" ]] \
  || die "continuation compute-side submission driver is unavailable"
driver="$(realpath -e -- "${driver}")"

export_names=("${required_environment[@]}" "${optional_environment[@]}")
